 ...
 * 将被转发/引用的帖子映射为前端所需格式。
 *
 * @param repostOf - 被转发的原始帖子
 * @returns 格式化后的引用帖子对象，若入参为空则返回 null
 */
function mapRepostOf(repostOf: QuotedPost | null | undefined) {
  if (!repostOf) {
    return null;
  }
  ...
  return {
    id: repostOf.id,
    author: mapAuthor(repostOf.author, false),
...
export function toFeedItem(post: PostWithRelations) {
  return {
    ...
    repostOf: mapRepostOf(post.repostOf)
  };
}
